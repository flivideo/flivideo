...
- schema-mirror: an **object whose values are zod schemas** (Teletubby `INPUT` in `src/core/input-shapes.ts:78` — every verb's input schema) is not read; the census lists it under "never read" (object constant), so it is disclosed, not silent. Extractor improvement: walk object-literal values that are zod schemas. **Second instance (FliHub 283935b):** object/array constants `NAMING_RULES`, `REFUSAL_CODES`, `DEFAULT_PROJECT_STAGES` are "declared but not read" — a README "see the mirror" pointer then leads nowhere. Extend to object and array constants of literals. Found in Teletubby re-run a5c44a7, verified in brains 2026-09-23 (all three gates exit 0).
